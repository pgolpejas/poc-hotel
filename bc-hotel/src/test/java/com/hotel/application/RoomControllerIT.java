...
import com.hotel.utils.BaseTestContainerFromDockerCompose;
...
class RoomControllerIT extends BaseTestContainerFromDockerCompose {
    ...
    class FindById {
        ...
        void when_room_exists_should_return_it() throws Exception {
            String roomId = "d1b97f69-7fa0-4301-b498-128d78860828";

            final ResponseEntity<RoomDto> response =
            ...
            RoomDto answer = response.getBody();
            Assertions.assertThat(answer).as("room")
                    .isNotNull();
        }

        @Test
        void when_room_not_exists_should_return_not_found() throws Exception {
    ...
    class Search {
        ...
        void when_roomTypeInventories_exists_filter_and_should_return_it() throws Exception {
            ...
            final ResponseEntity<RoomPaginationResponse> response =
                    restTemplate.exchange(MAPPING + SEARCH_PATH,
                            HttpMethod.POST,
                            RequestUtils.buildRequest(null, criteria),
                            RoomPaginationResponse.class,
                            criteria);

            Assertions.assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);

            RoomPaginationResponse answer = response.getBody();
    ...
    class SearchAudit {
        ...
        @Test
        void when_room_has_audit_return_it() throws Exception {
            ...
            int limit = 10;
            final ResponseEntity<RoomListResponse> response =
            ...
            RoomListResponse answer = response.getBody();
            Assertions.assertThat(answer).as("roomTypeInventories").isNotNull();
        }
    }
    ...
    class Create {
        ...
        @Test
        @Sql({"/sql/hotel/single.sql"})
        void when_room_should_create_it() throws Exception {
            UUID roomId = UUID.randomUUID();
            ...
            final ResponseEntity<RoomDto> response = restTemplate.exchange(MAPPING,
                    HttpMethod.POST,
                    RequestUtils.buildRequest(null, roomDTO),
                    RoomDto.class);
        ...
        @Test
        @Sql({"/sql/hotel/single.sql"})
        void when_room_incorrect_data_should_not_create_it() throws Exception {
            UUID roomId = UUID.randomUUID();
            final RoomDto roomDTO = new RoomDto()
                    .id(roomId)
                    .hotelId(HOTEL_ID)
        ...
        void when_room_already_exist_by_id_should_return_conflict() {
            UUID roomId = UUID.fromString("d1b97f69-7fa0-4301-b498-128d78860828");
            final RoomDto roomDTO = new RoomDto()
                    .id(roomId)
                    .hotelId(HOTEL_ID)
                    .roomTypeId(1)
            ...
            HttpEntity<?> request = new HttpEntity<>(roomDTO, headers);
    ...
    class Update {
        ...
        @Test
        void when_room_not_exists_should_not_found_exception() throws Exception {
            UUID roomId = UUID.randomUUID();
            final RoomDto roomDTO = new RoomDto()
                    .id(roomId)
                    .hotelId(HOTEL_ID)
        ...
        @Sql({"/sql/hotel/single.sql"})
        @Sql({"/sql/room/single.sql"})
        void when_room_exists_should_update_it() throws Exception {
            UUID roomId = UUID.fromString("d1b97f69-7fa0-4301-b498-128d78860828");
            final RoomDto roomDTO = new RoomDto()
                    .id(roomId)
                    .hotelId(HOTEL_ID)
    ...
    class DeleteIT {
        ...
        @Sql({"/sql/hotel/single.sql"})
        @Sql({"/sql/room/single.sql"})
        void when_room_exists_should_delete_it() throws Exception {
        ...
        @Test
        void when_room_not_exists_and_delete_should_return_not_found() throws Exception {
