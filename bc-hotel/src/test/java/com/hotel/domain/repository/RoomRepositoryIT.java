...
class RoomRepositoryIT extends BaseTestContainerFromDockerCompose {
  ...
  class Create {
    ...
    void when_room_is_ok_should_create_it() {
      ...
      Optional<Room> optSearch = roomRepository.findById(room.getId().value());
  ...
  class Search {
    ...
    void when_room_filter_with_selection_should_return_it() {
      ...
      final PaginationResponse<Room> pageResponse = roomRepository.searchBySelection(criteria);
    ...
    void when_room_filter_should_return_it() {
      ...
      final PaginationResponse<Room> pageResponse = roomRepository.search(criteria);
  ...
  class SearchAudit {
    ...
    void when_room_created_javers_audited_it_and_return_shadows() {
      ...
      Integer roomTypeId = 1;
      final Room room = Room.builder().id(id).hotelId(HOTEL_ID).roomTypeId(roomTypeId)
          .name("Room")
          .floor(1)
          .roomNumber("101")
          .available(true)
      ...
      final Room roomUpdate = Room.builder().id(id).hotelId(HOTEL_ID).roomTypeId(roomTypeId)
          .name("Room1")
          .floor(1)
          .roomNumber("102")
          .available(true)
      ...
      final Room roomUpdate2 = Room.builder().id(id).hotelId(HOTEL_ID).roomTypeId(roomTypeId)
          .name("Room31")
          .floor(12)
          .roomNumber("1032")
          .available(true)
          .build();
      roomRepository.save(roomUpdate2);
