...
public class OutboxEntityPostProcessorImpl implements OutboxEntityPostProcessor {
    ...
    public boolean process(final List<BaseEventEntity> sourceEntities, final boolean checkOlderEvents)
        ...
        List<String> ids = sourceEntities.stream().map(ent -> ent.getId().toString()).toList();
        try {
            results = processForEach(checkOlderEvents, sourceEntities);
        } catch (final Exception me) {
            throw new OutboxRuntimeException("Transactional process for entities {} failed , event not send:" + ids, me);
        }
    ...
    private Map<String, Boolean> processForEach(final boolean checkOlderEvents, List<BaseEventEntity> sourceEntities) throws JsonProcessingException {
        Map<String, Boolean> results = new HashMap<>();

        boolean continueProcessing = true;
        BaseEventEntity entity;
        if (checkOlderEvents) {
            List<String> ids = sourceEntities.stream().map(ent -> ent.getId().toString()).toList();
            entity = sourceEntities.getFirst();
            log.info("Check older events than:{}", ids);
            // Only check the first event, because all other events are for the same aggregate and transaction
            List<BaseEventEntity> entities = outboxRepositoryFactory.resolve(entity.getClass())
        ...
        if (continueProcessing) {
            ...
            for (BaseEventEntity partialEnt : sourceEntities) {
                if (continueSend) {
                    // The moment one fails, the sending is stopped, and they are queued for the scheduler.
                    continueSend = continueProcessingSend(partialEnt);
                }
    ...
    private boolean continueProcessingSend(BaseEventEntity entity) throws JsonProcessingException {
        ...
        SpecificRecord envelope = (SpecificRecord) this.deserializer.deserialize("", entity.getSerializedEvent());

        // Add headers
        Map<String, Object> headers = entity.getHeadersMap() != null
                ? mapper.readValue(entity.getHeadersMap(), new TypeReference<>() {
        })
                : new HashMap<>();
        Message<SpecificRecord> msgToSend = MessageBuilder.createMessage(envelope, new MessageHeaders(headers));

        boolean send = streamBridge.send(this.getBindingName(entity, envelope.getClass().getName()), msgToSend);
        log.info("Entity {} send {}", entity.getId(), send);

        EventPublisherResult publishResult = EventPublisherResult.builder()
                .retryable(true)
                .successful(send)
                .build();
        return processResult(publishResult, entity, watch);
    }
    ...
    private String getBindingName(BaseEventEntity entity, String envelopeClassName) {
        ...
        OutboxConfiguration.EventBinding bind = configuration.getEventBinding().get(envelopeClassName);
        if (bind == null) {
            final String message = String.format("Processing for entity %s was failed, event not send: %s there is no binding configuration for event type: %s", entity, envelopeClassName, entity.getAggregateType());
            log.error(message);
    ...
    protected boolean processResult(EventPublisherResult publishResult, BaseEventEntity entityToSave, StopWatch watch) {
        ...
        OutboxRepository outboxRepository = getOutboxRepositoryFactory().resolve(entityToSave.getClass());

        if (publishResult.isSuccessful()) {

            watch.start();
            if (getConfiguration().isDeleteEventOnSend(entityToSave.getClass())) {
