...
class RoomTypeInventoryRepositoryIT extends BaseTestContainerFromDockerCompose {
    ...
    class Create {
        ...
        void when_roomTypeInventory_is_ok_should_create_it() {
            ...
            final RoomTypeInventory roomTypeInventory =
                    RoomTypeInventory.builder()
                            .id(UUID.randomUUID())
                            .hotelId(hotelId)
                            .roomTypeId(roomTypeId)
                            .roomTypeInventoryDate(LocalDate.now())
                            .totalInventory(1)
                            .totalReserved(1)
                            .build();
            roomTypeInventoryRepository.save(roomTypeInventory);

            Optional<RoomTypeInventory> optSearch = roomTypeInventoryRepository.findById(roomTypeInventory.getId().value());
            ...
            assertThat(optSearch.get().id().toString()).as("aggregateId").hasToString(roomTypeInventory.getAggregateId());
            ...
            assertThat(optSearch.get().totalInventory()).as("totalInventory").isEqualTo(roomTypeInventory.totalInventory());
            assertThat(optSearch.get().totalReserved()).as("totalReserved").isEqualTo(roomTypeInventory.totalReserved());
        }
    }
    ...
    class Search {
        ...
        void when_roomTypeInventory_filter_with_selection_should_return_it() {
            ...
            final PaginationResponse<RoomTypeInventory> pageResponse = roomTypeInventoryRepository.searchBySelection(criteria);
            ...
            assertThat(pageResponse.data().getFirst().getAggregateId()).as("aggregateId").hasToString(roomTypeInventoryId);
        ...
        void when_roomTypeInventory_filter_should_return_it() {
            ...
            final PaginationResponse<RoomTypeInventory> pageResponse = roomTypeInventoryRepository.search(criteria);
    ...
    class SearchAudit {
        ...
        void when_roomTypeInventory_created_javers_audited_it_and_return_shadows() {
            ...
            UUID hotelId = UUID.randomUUID();
            Integer roomTypeId = 1;
            final RoomTypeInventory roomTypeInventory = RoomTypeInventory.builder().id(id).hotelId(hotelId).roomTypeId(roomTypeId)
                    .roomTypeInventoryDate(LocalDate.now())
                    .totalInventory(1)
                    .totalReserved(1)
                    .build();
            ...
            final RoomTypeInventory roomTypeInventoryUpdate = RoomTypeInventory.builder().id(id).hotelId(hotelId).roomTypeId(roomTypeId)
                    .roomTypeInventoryDate(LocalDate.now())
                    .totalInventory(2)
                    .totalReserved(1)
                    .build();
            roomTypeInventoryRepository.save(roomTypeInventoryUpdate);

            final RoomTypeInventory roomTypeInventoryUpdate2 = RoomTypeInventory.builder().id(id).hotelId(hotelId).roomTypeId(roomTypeId)
                    .roomTypeInventoryDate(LocalDate.now())
                    .totalInventory(2)
                    .totalReserved(2)
                    .build();
            roomTypeInventoryRepository.save(roomTypeInventoryUpdate2);

            roomTypeInventoryRepository.delete(roomTypeInventory);
            ...
            final AuditFilters filters = AuditFilters.builder()
                    ...
                    .build();
            final List<RoomTypeInventory> plannedProductList = RoomTypeInventoryRepositoryIT.this.roomTypeInventoryRepository.findAuditByFilters(filters, 10);
