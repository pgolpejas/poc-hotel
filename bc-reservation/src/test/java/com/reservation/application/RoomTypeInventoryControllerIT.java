...
class RoomTypeInventoryControllerIT extends BaseTestContainerFromDockerCompose {
  ...
  class FindById {
    ...
    void when_roomTypeInventory_exists_should_return_it() throws Exception {
      String roomTypeInventoryId = "c1a97f69-7fa0-4301-b498-128d78860828";

      final ResponseEntity<RoomTypeInventoryDto> response =
      ...
      RoomTypeInventoryDto answer = response.getBody();
      Assertions.assertThat(answer).as("roomTypeInventory")
          .isNotNull();
    }

    @Test
    void when_roomTypeInventory_not_exists_should_return_not_found() throws Exception {
      String roomTypeInventoryId = UUID.randomUUID().toString();
  ...
  class Search {
    ...
    void when_roomTypeInventories_exists_filter_and_should_return_it() throws Exception {
      ...
      final ResponseEntity<RoomTypeInventoryPaginationResponse> response =
          restTemplate.exchange(MAPPING + SEARCH_PATH,
              HttpMethod.POST,
              RequestUtils.buildRequest(null, criteria),
              RoomTypeInventoryPaginationResponse.class,
              criteria);

      Assertions.assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);

      RoomTypeInventoryPaginationResponse answer = response.getBody();
  ...
  class SearchAudit {
    ...
    void when_roomTypeInventory_has_audit_return_it() throws Exception {
      UUID roomTypeInventoryId = UUID.randomUUID();
      final AuditFilters filters = AuditFilters.builder()
          .id(roomTypeInventoryId)
          .build();

      int limit = 10;
      final ResponseEntity<RoomTypeInventoryListResponse> response =
      ...
      RoomTypeInventoryListResponse answer = response.getBody();
      Assertions.assertThat(answer).as("roomTypeInventories").isNotNull();
    }
  }
  ...
  class Create {
    ...
    void when_roomTypeInventory_should_create_it() throws Exception {
      UUID roomTypeInventoryId = UUID.randomUUID();
      final RoomTypeInventoryDto roomTypeInventoryDTO = new RoomTypeInventoryDto()
          .id(roomTypeInventoryId)
      ...
      final ResponseEntity<RoomTypeInventoryDto> response = restTemplate.exchange(MAPPING,
          HttpMethod.POST,
          RequestUtils.buildRequest(null, roomTypeInventoryDTO),
          RoomTypeInventoryDto.class);
    ...
    void when_roomTypeInventory_already_exist_by_id_should_return_conflict() {
      UUID roomTypeInventoryId = UUID.fromString("c1a97f69-7fa0-4301-b498-128d78860828");
      final RoomTypeInventoryDto roomTypeInventoryDTO = new RoomTypeInventoryDto()
          .id(roomTypeInventoryId)
          .hotelId(UUID.randomUUID())
          .roomTypeId(1)
      ...
      HttpEntity<?> request = new HttpEntity<>(roomTypeInventoryDTO, headers);
    ...
    void when_roomTypeInventory_already_exist_by_uk_should_return_conflict() {
      final RoomTypeInventoryDto roomTypeInventoryDTO = new RoomTypeInventoryDto()
          ...
          .roomTypeId(1)
          .roomTypeInventoryDate(LocalDate.of(2024, 1, 30))
          .totalInventory(10)
          .totalReserved(1);
