...
import com.reservation.domain.usecase.UpdateRoomTypeInventoryUseCase;
...
public class UpdateRoomTypeInventoryUseCaseImpl implements UpdateRoomTypeInventoryUseCase {
    ...
    public void updateRoomTypeInventory(final RoomTypeInventory roomTypeInventory) {
        ...
        Optional<RoomTypeInventory> optionalRoomTypeInventory = repository.findById(roomTypeInventory.id());

        if (optionalRoomTypeInventory.isEmpty()) {
            throw new RoomTypeInventoryNotFoundException("RoomTypeInventory with id %s not found", roomTypeInventory.id().toString());
        } else {
