...
@Validated
public class CreateReservationCommandHandler implements CreateReservationCommand {
	...
	public void createReservation(@Valid final Reservation reservation) {
		...
		if (this.reservationRepository.existsById(reservation.id())) {
			throw new ReservationConflictException("Reservation with id %s already exists",
					reservation.id().toString());
		}

		final Reservation newReservation = Reservation.create(reservation);

		final List<RoomTypeInventory> roomTypeInventories = this.roomTypeInventoryRepository.findByReservationDates(
				reservation.hotelId(), reservation.roomTypeId(), reservation.start(), reservation.end());
		if (!this.hasAvailability(roomTypeInventories, reservation.start(), reservation.end())) {
			throw new ReservationConflictException("Reservation [%s,%s,%s,%s] has not availability",
					reservation.hotelId(), reservation.roomTypeId(), reservation.start(), reservation.end());
		}
