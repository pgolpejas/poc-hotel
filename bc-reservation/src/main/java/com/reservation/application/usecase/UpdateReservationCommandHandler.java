...
public class UpdateReservationCommandHandler implements UpdateReservationCommand {
	...
	public void updateReservation(final Reservation reservation) {
		...
		final Optional<Reservation> optionalReservation = this.reservationRepository.findById(reservation.id());

		if (optionalReservation.isEmpty()) {
			throw new ReservationNotFoundException("Reservation with id %s not found", reservation.id().toString());
		} else {
			final Reservation aggregate = optionalReservation.get();

			if (!reservation.start().isEqual(aggregate.start()) || !reservation.end().isEqual(aggregate.end())) {
				this.roomTypeInventoryRepository.findByReservationDates(aggregate.hotelId(), aggregate.roomTypeId(),
						aggregate.start(), aggregate.end()).forEach(roomTypeInventory -> {
