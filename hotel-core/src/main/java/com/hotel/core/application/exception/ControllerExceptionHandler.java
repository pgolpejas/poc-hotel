...
public class ControllerExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<ProblemDetail> handleConflictException(final HttpServletRequest request,
			final ConflictException exception) {
		final HttpStatus status = HttpStatus.CONFLICT;
		return ResponseEntity.status(status).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_PROBLEM_JSON_VALUE)
				.body(this.build(status, exception.getDetail(), request.getContextPath() + request.getServletPath()));
	}

	@ExceptionHandler(NotFoundException.class)
	public ResponseEntity<ProblemDetail> handleNotFoundException(final HttpServletRequest request,
	...
	public ResponseEntity<ProblemDetail> handleDomainError(final HttpServletRequest request,
			final DomainError exception) {
		final HttpStatus status = HttpStatus.BAD_REQUEST;
		return ResponseEntity.status(status).header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_PROBLEM_JSON_VALUE)
	...
	protected @Nullable ResponseEntity<Object> handleMethodArgumentNotValid(final MethodArgumentNotValidException ex,
		...
		if (null != response && response.getBody() instanceof ProblemDetail problemDetailBody) {
			...
			final String errorDetails = bindingResult.getFieldErrors().stream()
